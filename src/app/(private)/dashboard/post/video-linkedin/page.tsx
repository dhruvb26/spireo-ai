...
import { z } from "zod";
import { RepurposeFormSchema } from "@/components/forms/video-form";
...
const VideoLinkedInPage = () => {
  ...
  const handleSubmit = async (data: formContent) => {
    ...
    try {
      const response = await fetch("/api/ai/repurpose-content/transcribe", {
        method: "POST",
        headers: {
    ...
    <main>
      <div className="mb-8">
        <h1 className="text-xl font-semibold tracking-tight text-brand-gray-900">
          Convert any Video to LinkedIn Post
        </h1>
